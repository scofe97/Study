...
def extract3(filename):
    ...
    for row in extract_ws.iter_rows(min_row=2):
        ...
        novel_VEL = re.findall(r'(?:\d*,)*\d+명.*회',row[1].value)[0].replace("\xa0","").split()
        ...
        novel_code = int(row[4].value.replace("https://novelpia.com/novel/",""))
        
        novel_start_30 = []
        novel_end_30 = []
        if len(row) > 5:
            if row[5].value != '' and row[5].value != None:
                novel_start_episodes = row[5].value.replace("\t","").replace("\n","").replace("\xa0","").replace("x000D","  ").replace("x000d","  ").replace("_","").split("  ")
            ...
            if row[6].value != '' and row[6].value != None:
                novel_end_episodes = row[6].value.replace("\t","").replace("\n","").replace("\xa0","").replace("x000D","  ").replace("x000d","  ").replace("_","").split("  ")
        ...
        on_hiatu_novel = [96550, 97252, 96314, 96395, 96510, 96998, 96432]
        
        if novel_code in on_hiatu_novel:
            continue
